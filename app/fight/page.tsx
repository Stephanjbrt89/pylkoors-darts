...
export default function FightGamePage() {
  ...
  const handleStartGame = async (selectedPlayers: any[]) => {
    ...
    try {
      ...
      // Auto-assign random sectors immediately
      const availableNumbers = Array.from({ length: 20 }, (_, i) => i + 1).sort(() => Math.random() - 0.5);
      const finalPlayers = initial.players.map(p => ({ 
        ...p, 
        sectors: [availableNumbers.pop()!, availableNumbers.pop()!] 
      }));
  ...
  const handleThrow = async (score: number, multiplier: number, label: string) => {
   ...
   if (nextState.isFinished && nextState.winnerId) {
    ...
    if (winner) {
        const accuracy = Math.round((winner.effectiveDarts / winner.totalDarts) * 100);
        await StatsService.updateRecord('ARCADE_FIGHT', 'ACCURACY', winner.id, accuracy, matchId);
    }
}
  ...
  return (
    <main className="h-screen w-full grid grid-rows-[60px_1fr_min-content] bg-[#020205] text-white overflow-hidden relative transition-all duration-500">
      ...
      <div className="w-full flex justify-between items-center px-10 py-2 relative z-20 bg-black/40 border-b border-white/5">
        ...
        <GameInfo title="Fight Game" color="#ff003c" rules={["Defend sectors.", "Hit opponent sectors to damage.", "Hit your own to heal.", "Multipliers count!"]} />
      </div>

      {/* 2. THE ARENA - Center Stage (No bottom padding) */}
      <div className="relative z-10 flex items-end justify-center px-10 pb-0 overflow-visible h-full">
        <div className="flex justify-around items-end w-full max-w-[1700px] h-full pb-2">
          {gameState.players.map((p, idx) => (
            <div key={p.id} className="relative flex-1 flex justify-center h-full items-end">
              <GooierFighter 
                player={p} 
                isActive={idx === gameState.currentTurnIndex} 
                isDamaged={impactPlayerId === p.id} 
                isHealed={healPlayerId === p.id} 
              />
              
              {impactPlayerId === p.id && impactData && (
                <div className="absolute top-[35%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100] pointer-events-none text-center">
      ...
      {/* 3. CONSOLE - Anchored to bottom, height auto-calculated */}
      {!gameState.isFinished && (
        <div className="w-full flex justify-center z-30 pb-0">
          <ArcadeNumpad 
      ...
      {gameState.isFinished && winner && (
          <div className="fixed inset-0 z-[1000] bg-black/95 flex items-center justify-center p-6 animate-in zoom-in">
              <div className="relative bg-[#0a0a1a] border-4 border-red-600 p-10 rounded-[3rem] max-w-xl w-full text-center shadow-2xl">
                ...
                <img src={winner.avatar_url} className="w-32 h-32 rounded-full border-4 border-yellow-500 mx-auto mb-4 object-cover bg-black" alt="" />
                <p className="text-3xl font-black text-white uppercase mb-8 italic">{winner.username}</p>
