    ...
    <main className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-white text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
        <button onClick={() => handleStartGame('EASY')} className="bg-slate-900 border-4 border-slate-800 p-10 rounded-[3rem] text-left hover:border-yellow-500 transition-all">
          ...
          <p className="text-slate-400 text-sm italic font-bold">All doubles/triples count.</p>
        </button>
        <button onClick={() => handleStartGame('HARD')} className="bg-slate-900 border-4 border-slate-800 p-10 rounded-[3rem] text-left hover:border-red-600 transition-all">
          ...
          <p className="text-slate-400 text-sm italic font-bold">Only 10-20 Multipliers count.</p>
        </button>
      </div>
    </main>
  );
  ...
  return (
    <main className="h-screen w-full grid grid-rows-[60px_1fr_min-content] bg-[#020205] text-white overflow-hidden relative">
        ...
        <div className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center p-6">
          ...
          <div className="flex gap-8">
            {gameState.pendingChoice.options.map(opt => (
              <button key={opt} onClick={() => handleChoice(opt)} className="bg-white text-black px-20 py-10 rounded-[2.5rem] font-black text-6xl hover:scale-110 transition border-8 border-yellow-500 italic uppercase">{opt}</button>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="w-full flex justify-between items-center px-10 py-2 bg-black/40 border-b border-white/5 z-20">
        <Link href="/" className="bg-slate-900 border border-slate-700 px-6 py-2 rounded-full text-[10px] font-black uppercase hover:bg-red-600 transition text-white shadow-lg">LOBBY</Link>
        ...
        <div className={`h-full w-full bg-black/60 rounded-[2.5rem] border-4 border-slate-800 shadow-2xl flex flex-col overflow-hidden ${isSolo ? 'max-w-md' : 'max-w-[1300px]'}`}>
            {!isSolo && (
              <div className={`bg-[#111] grid items-center text-center py-2 border-b-2 border-yellow-500 ${gridCols}`}>
                <div className="flex flex-col items-center"><img src={gameState.players[0].avatar_url} className="w-14 h-14 rounded-full border-4 border-yellow-500 object-cover bg-black" /><p className="text-[10px] font-black mt-1 text-yellow-500 uppercase leading-none">{gameState.players[0].username}</p></div>
                {!isSolo && !isVersus && (
                    <div className="flex flex-col items-center"><img src={gameState.players[1]?.avatar_url} className="w-14 h-14 rounded-full border-2 border-white/20 object-cover bg-black" /><p className="text-[8px] font-black mt-1 text-slate-500 uppercase leading-none">{gameState.players[1]?.username}</p></div>
                )}
                <div className="font-black italic text-yellow-500 text-xl tracking-[0.2em] uppercase">Target</div>
                <div className="flex flex-col items-center">{isVersus ? <><img src={gameState.players[1].avatar_url} className="w-14 h-14 rounded-full border-4 border-yellow-500 object-cover bg-black" /><p className="text-[10px] font-black mt-1 text-yellow-500 uppercase leading-none">{gameState.players[1].username}</p></> : (gameState.players[2] && <><img src={gameState.players[2].avatar_url} className="w-14 h-14 rounded-full border-2 border-white/20 object-cover bg-black" /><p className="text-[8px] font-black mt-1 text-slate-500 uppercase leading-none">{gameState.players[2].username}</p></>)}</div>
                {!isSolo && !isVersus && <div className="flex flex-col items-center">{gameState.players[3] && <><img src={gameState.players[3].avatar_url} className="w-14 h-14 rounded-full border-2 border-white/20 object-cover bg-black" /><p className="text-[8px] font-black mt-1 text-slate-500 uppercase leading-none">{gameState.players[3].username}</p></>}</div>}
              </div>
            )}
                ...
                {KILLER_TARGETS.map((target) => {
                    ...
                    return (
                        <div key={tStr} className={`grid flex-1 items-center ${gridCols} ${isDead ? 'opacity-10 grayscale bg-red-950/20' : ''}`}>
                            <div className="flex justify-center gap-1.5">{[1, 2, 3].map(n => <div key={n} className={`text-2xl font-black transition-all ${gameState.players[0].hits[tStr] >= n ? 'text-yellow-500 drop-shadow-[0_0_8px_#facc15]' : 'text-slate-900'}`}>{gameState.players[0].hits[tStr] >= n ? 'X' : '•'}</div>)}</div>
                            {!isSolo && !isVersus && <div className="flex justify-center gap-1.5">{gameState.players[1] ? [1, 2, 3].map(n => <div key={n} className={`text-2xl font-black ${gameState.players[1].hits[tStr] >= n ? 'text-yellow-500' : 'text-slate-900'}`}>{gameState.players[1].hits[tStr] >= n ? 'X' : '•'}</div>) : null}</div>}
                            <div className={`text-2xl font-black italic text-center border-x border-white/10 bg-black/40 py-0.5 ${isDead ? 'line-through text-slate-700' : 'text-yellow-400'}`}>{target}</div>
                            <div className="flex justify-center gap-1.5">{isVersus ? ([1, 2, 3].map(n => <div key={n} className={`text-2xl font-black transition-all ${gameState.players[1].hits[tStr] >= n ? 'text-yellow-500 drop-shadow-[0_0_8px_#facc15]' : 'text-slate-900'}`}>{gameState.players[1].hits[tStr] >= n ? 'X' : '•'}</div>)) : (gameState.players[2] ? [1, 2, 3].map(n => <div key={n} className={`text-2xl font-black ${gameState.players[2].hits[tStr] >= n ? 'text-yellow-500' : 'text-slate-900'}`}>{gameState.players[2].hits[tStr] >= n ? 'X' : '•'}</div>) : null)}</div>
                            {!isSolo && !isVersus && <div className="flex justify-center gap-1.5">{gameState.players[3] ? [1, 2, 3].map(n => <div key={n} className={`text-2xl font-black ${gameState.players[3].hits[tStr] >= n ? 'text-yellow-500' : 'text-slate-900'}`}>{gameState.players[3].hits[tStr] >= n ? 'X' : '•'}</div>) : null}</div>}
                        </div>
                    );
                })}
            </div>
      ...
      {!gameState.isFinished && (
        <div className="w-full flex justify-center z-40 relative">
          <ArcadeNumpad activeGooierName={currentPlayer.username} dartsThrownCount={gameState.dartsThrown.length} onThrow={onHit} onEndTurn={() => setGameState(prev => ({ ...prev!, currentTurnIndex: (prev!.currentTurnIndex + 1) % prev!.players.length, dartsThrown: [] }))} color="#facc15" />
        </div>
      )}
      ...
      {gameState.isFinished && (
          <div className="fixed inset-0 z-[1000] bg-black/95 flex items-center justify-center p-6 animate-in zoom-in">
              <div className="relative bg-[#0a0a1a] border-4 border-yellow-500 p-12 rounded-[4rem] max-w-xl w-full text-center shadow-2xl">
                <h2 className="text-8xl font-black italic text-yellow-500 uppercase mb-8 tracking-tighter">{isSolo ? 'MASTERED' : 'CONQUERED'}</h2>
                <img src={gameState.players.find(p => p.id === gameState.winnerId)?.avatar_url} className="w-40 h-40 rounded-full border-8 border-yellow-500 mx-auto mb-6 object-cover bg-black shadow-2xl" alt="" />
